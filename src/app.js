...
function createApp({ printer = require('./services/printer.service'), port = 4000, allowedOrigins = [] } = {}) {
  const app = express();
  ...
  app.use('/print', createPrintRoutes(printer));
  app.get('/health', (_req, res) => res.json({ service: 'printer-server', status: 'ok', printerConnected: printer.isPrinterOpen() }));
